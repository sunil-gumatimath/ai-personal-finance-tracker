...
import { buildUpdateQuery, buildInsertQuery } from "../services/query-builder.js";
  ...
  if (req.method === "POST") {
    try {
      ...
      if (!catType || !["income", "expense"].includes(catType)) {
        res
          .status(400)
          .json({ error: "Valid category type is required (income, expense)" });
        return;
      }

      const queryData = buildInsertQuery("categories", data, {
        user_id: userId,
      });
      const { rows } = await query(queryData.text, queryData.values);
