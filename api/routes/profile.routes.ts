...
import { buildUpdateQuery } from "../repositories/_query-builder.js"
import {
  decryptPreferences,
  encryptPreferences,
...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  if (req.method === 'GET') {
    try {
      ...
      res.status(200).json({
        preferences: sanitizePreferencesForClient(decryptedPrefs),
  ...
  if (req.method === 'PATCH') {
    try {
      ...
      const decryptedIncoming = { ...(preferences || {}) }
      delete decryptedIncoming.geminiApiKey
      delete decryptedIncoming.openrouterApiKey
      delete decryptedIncoming.geminiApiKeyConfigured
      delete decryptedIncoming.openrouterApiKeyConfigured
      ...
      if (apiKeys && typeof apiKeys === 'object' && !Array.isArray(apiKeys)) {
        for (const key of ['geminiApiKey', 'openrouterApiKey'] as const) {
          const value = apiKeys[key]
          if (value === null) {
            delete mergedPlaintext[key]
          } else if (typeof value === 'string' && value.trim()) {
            mergedPlaintext[key] = value.trim()
          }
        }
      }
      ...
      res.status(200).json({
        ok: true,
        preferences: sanitizePreferencesForClient(mergedPlaintext),
