...
export function sanitizePreferencesForClient(
  ...
  const geminiKey = sanitized.geminiApiKey;
  const openrouterKey = sanitized.openrouterApiKey;
  delete sanitized.geminiApiKey;
  delete sanitized.openrouterApiKey;
  sanitized.geminiApiKeyConfigured =
    typeof geminiKey === 'string' && geminiKey.trim().length > 0;
  sanitized.openrouterApiKeyConfigured =
