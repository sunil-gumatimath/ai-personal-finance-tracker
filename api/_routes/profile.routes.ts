...
import { buildUpdateQuery } from "../_repositories/_query-builder.js"
import {
  decryptPreferences,
  encryptPreferences,
...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  if (req.method === 'PATCH') {
    try {
      ...
      delete decryptedIncoming.kilocodeApiKey
      delete decryptedIncoming.kilocodeApiKeyConfigured
      ...
      if (apiKeys && typeof apiKeys === 'object' && !Array.isArray(apiKeys)) {
        for (const key of ['kilocodeApiKey'] as const) {
