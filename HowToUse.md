...
- Paste your Kilo Gateway API key. You can toggle the key visibility (show/hide) using the eye icon next to the input field.
- Pick a model from the curated free-model dropdown (default `nvidia/nemotron-3-ultra-550b-a55b:free`).
...
Open **Reports** from the sidebar to view monthly or trailing-12-month summaries: income vs. expenses, savings rate, spending by category, account balances, and transactions. Export the current period as a formatted **PDF** or **CSV**.
...
- Configure your KiloCode API key and model.
